...
if __name__ == "__main__":
    ...
    _check_item = config.get("ACFUN", [])[0]
    pushToken = config.get("PUSHTOKEN", [])
    if pushToken != None:
        msg1 = AcFun(check_item=_check_item).main()
        content = "今日AcFun签到！\n"+msg1
        requests.post('https://www.pushplus.plus/send', { 'token': pushToken, 'title': 'AcFun签到', 'content': content });
